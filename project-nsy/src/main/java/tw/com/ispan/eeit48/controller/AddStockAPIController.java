...
//		}
	...
	@PostMapping(path = { "/showproduct" })
	public void SelectProduct(@RequestBody ProductBean dataRequest, HttpSession session) {
		AccountsBean beanofaccount = (AccountsBean) session.getAttribute("user");
		List<ProductBean> beans = setNewStockService.SelectProduct(dataRequest.getProductid(),
				beanofaccount.getAccountid());
